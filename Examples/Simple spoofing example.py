...
# spoofing of an RS41 radiosonde with an    #
# RS41-SG/P radiosonde simulation framework.#
# The code loads messages from an RS41 log  #
# file and changes its identification data. #
# It also updates its GPS data and          #
# transmits the new message (via an audio   #
# channel or an RF transmitter) in one-     #
# second intervals. A sounding receiver     #
# receiving these messages will show that   #
# the spoofed radiosonde is flying right    #
# now.                                      #
#                                           #
...
RadiosondeID = "N4341111"
MainboardSerial = "N3362222"
PressureSerial = "N4243333"

GPSTimeLeapseconds = 18
GPSLatitudeNOffset = -6.22
GPSLongitudeEOffset = 45.47
...
# Get the GPS satellites file from the internet
SkyfieldSatellites = load.tle_file('http://celestrak.org/NORAD/elements/gps-ops.txt')

# Shuflle the satellites, to later avoid a constant SV's list in every run
random.shuffle(SkyfieldSatellites)
...
while (LoggedMessageIndex < LoggedMessagesLength) and (DebugTimeDiff < DebugSeconds):
    ...
    if (UTCTimeMicroseconds < 50000) & (UTCTime >= TxPreparationUTCTime):
        ...
        if CheckSTATUSblockCRC(MessageBytes):
            ...
            # Set fake data in message byte array
            SetRadiosondeID(RadiosondeID, MessageBytes)
            SetMainboardSerialInMessage(MainboardSerial, SubframeNumber, MessageBytes)
            SetPressureSerialInMessage(PressureSerial, SubframeNumber, MessageBytes)
            SetTxFrequencyInMessage(TxFrequency, SubframeNumber, MessageBytes)
        ...
        if CheckGPSPOSblockCRC(MessageBytes):
            ...
            GPSVelEast, GPSVelNorth, GPSVelUp = pymap3d.ecef2enuv(ECEFVelocityX, ECEFVelocityY, ECEFVelocityZ, GPSLatitudeN, GPSLongitudeE)               

            # Get GPS week number, GPS time in seconds and GPS time in milliseconds
            GPSWeek,_,_,GPSMilliseconds = UTCTimeToGPSWeekAndSeconds(TxUTCTime ,GPSTimeLeapseconds)

            # Apply the offsets on GPSLatitudeN, GPSLongitudeE
            GPSLatitudeN  = GPSLatitudeN  + GPSLatitudeNOffset
            GPSLongitudeE = GPSLongitudeE + GPSLongitudeEOffset
            ...
            (_, NumberOfSVs,
             ...
             GPSPDOP, MinPR,
             ECEFPositionX, ECEFPositionY, ECEFPositionZ, 
             ECEFVelocityX, ECEFVelocityY, ECEFVelocityZ) = CalculateGPSData(SkyfieldSatellites, TxUTCTime,
                                                                             GPSLatitudeN, GPSLongitudeE, GPSAltitude,
                                                                             GPSVelEast, GPSVelNorth, GPSVelUp)

            # Build a new GPSINFO block
            BuildGPSINFOblock(GPSWeek, GPSMilliseconds, PRNandReceptionQualityIndicatorArray, MessageBytes)                                                                 

            # Build a new GPSRAW block
            BuildGPSRAWblock(MinPR, PsaudorangeandVelocityArray, MessageBytes)

            # Build a new GPSPOS block
            BuildGPSPOSblock(ECEFPositionX, ECEFPositionY, ECEFPositionZ,
                                 ECEFVelocityX, ECEFVelocityY, ECEFVelocityZ,
                                 NumberOfSVs, GPSsAcc, GPSPDOP, MessageBytes)

        # If the Reed-Solomon decoding was OK, encode the message
        # An errornous message should not be encoded
        if RSRecovered:
            SetReedSolomon(MessageBytes)
